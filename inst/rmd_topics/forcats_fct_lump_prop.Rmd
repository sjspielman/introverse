...
#  factor variable. We will convert `order` (which will have many levels!) to a factor with base R function`as.factor()`.
msleep %>%
  drop_na(order) %>%                  # Remove any NAs from the vore column with tidyr::drop_na()
  mutate(order = as.factor(order)) %>% # Make the order column factor type with dplyr::mutate() 
  #Make the `order` column appear first for easier viewing with dplyr::select() and tidyselect::everything()
  select(order, everything()) -> msleep_fctorder

# Show the example dataset, which now shows `order` as <fct> (factor!) type
msleep_fctorder

# Show the default order (alphabetical!) and associated counts of the factor `order` column, 
# using dplyr::count() and dplyr::arrange()
# Examples below will manipulate the level order of the `order` column
msleep_fctorder %>%
  count(order) %>%
  arrange(n)
```
