...
The `aes()` function is used within `{ggplot2}` plots to **establish aesthetic _mappings_** between visual features in the plot and _columns_ in the tibble (data frame) being plotted. The `aes()` function should be used _only_ to map columns to aesthetics. Other stylings can be directly provided to geoms, without enclosing them in `aes()`.
...
```{r, example1}
# Place `vore` on the x-axis and `awake` on the y-axis, giving aes() to ggplot()
...
```{r, example2}
# Place `vore` on the x-axis and `awake` on the y-axis, with aes() on its own
# Note, to remove NAs, you must manipulate the data first
ggplot(msleep) + 
  aes(x = vore, y = awake) + 
  geom_boxplot()
