...
The first five examples below use the `msleep` dataset. Learn more about this dataset with `get_help("msleep")`.
...
```{r example2.5}
# Use contains() to REMOVE columns that contain the letter "w"
msleep %>%
  select(-contains("w"))
```
