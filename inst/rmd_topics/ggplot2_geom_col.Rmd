...
```{r example5}
# colplot of counts of categories in the vore column, using forcats::fct_infreq() to automatically order vores by frequency on the x-axis
# It best practice to also update the x-axis title 
...
## CAUTION!! `geom_col()` can only take either `x` or `y` in its default (non-advanced) usage 
...
```{r example7, error=TRUE}
## CAUTION!! `geom_col()` is used to plot categorical data, NOT numeric data!
## Plotting a numeric continuous variable results in confusion. 
## When plotting continuous data as cols, use `ggplot2::geom_histogram()` instead
ggplot(msleep) + 
  aes(x = awake) + 
  geom_col() 
```