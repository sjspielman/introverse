...
This function is used to *bind* the columns of two tibbles (data frames) or arrays into just tibble (data frame). This requires every column to be of equal length.
...
tibble1 <- select(msleep, name:vore) 
# dplyr::select() is extracting just the columns "name" to "vore", which includes "genus"
tibble2 <- select(msleep, order:sleep_total)
