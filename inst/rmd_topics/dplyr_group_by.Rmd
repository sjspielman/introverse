---
title: "`dplyr::group_by() and dplyr::ungroup()`"
...
`group_by()` establishes groupings of rows based on given columns for subsequent operations, and it is often (but not only!) used with the `{dplyr}` package `summarize()`. After you are done with your grouped operations, it is often a good idea use `ungroup()` (no arguments needed) to remove groupings and avoid unintended consequences of grouped data.
