...
When you run this code, you will see that only three rows remain. Those rows are the only rows in the data which are "setosa" species irises.
...
The preferred style is to use `filter()` with a pipe, using multiple lines to make our code easier to see:
...
Or, if we want rows where _either_ option is `TRUE`, we use the symbol `|`:
