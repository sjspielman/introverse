...
Confirm that there are 87 different Star Wars characters in the `starwars` data set.
...
Columns can be *classified* by character, numeric (real numbers), integer, complex, logical (True/False)
...
Write R code to ask what the maximum and minimum values are for horsepower, `hp`, within the `mtcars` data set are. 
...
Using the `iris` data set and `range()`, find out which numeric column has the largest range.
...
#It's both sepal width and petal width!
```

### Exercise 15

The numeric columns within `iris` can be made into arrays. Create an array named `ex15_array` from the column `Petal.Width` and write R code to ask for the summary statistics, using `summary()`. 

Compare your maximum and minimum to your answer to the previous exercise!
