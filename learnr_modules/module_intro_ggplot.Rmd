...
sparrows %>%
  dplyr::group_by(Sex) %>%
  dplyr::summarize(mean_wingspread = mean(Wingspread)) -> sparrow_meanwing_sex


sparrows %>%
  dplyr::count(Sex) -> sparrows_sex_precounted
sparrows %>%
  dplyr::count(Age) -> sparrows_age_precounted
sparrows %>%
  dplyr::count(Survival) -> sparrows_survival_precounted
sparrows %>%
  dplyr::count(Age, Sex, Survival) -> sparrows_precounted
```
...
# RE-DEFINE the order like this. Run this code to see:
# This *permanently* changes the Age column in sparrows, unless you change it again later
sparrows$Sex <- factor(sparrows$Age, levels=c("Female", "Male"))
...
The most common approach visualizing amounts, or count, data is with barplots, which we will focus on in this set. There are TWO commonly used geoms for making barplots:

+ `geom_bar()` will *count data for you* into a barplot and produce your Y-axis for you. This geom ONLY takes an X or Y mapping (use Y mapping for *horizontal* barplot)

+ `geom_col()` will draw a bar (column) whose height is precisely provided in the data set. This geom requires BOTH X AND Y mappings.
...
head(sparrows)
# Make a barplot of the NUMBER OF EACH SEX. Requires ggplot to count for us
ggplot(sparrows, aes(x = Sex)) + 
  geom_bar()
# Make a barplot of literally these averages in this other dataset I made for you:
sparrow_meanwing_sex
ggplot(sparrow_meanwing_sex, aes(x = Sex, y = mean_wingspread)) + 
  geom_col()
```

1. Using the full `sparrows` dataset, make a barplot using `geom_bar()` of the number of each *age group* (`Age`) in the dataset.
...
2. Using the full `sparrows` dataset, again make a barplot using `geom_bar()` of the number of each *age group* (`Age`) in the dataset. This time, add a fill aesthetic mapping to `Survival`. What do you notice about the result?
```{r bar_2, exercise = TRUE}

```

```{r bar_2-solution}
# We now see TWO bars for each age group, based on survival status
ggplot(sparrows, aes(x = Age, fill = Survival)) +
  geom_bar()
```


3. As you see in the question above, the default way that multiple bars will be shown is *stacked*. Modify the plot to show *dodged* bars using the argument `position = position_dodge()` in `geom_bar()`.
```{r bar_3, exercise = TRUE}

```

```{r bar_3-solution}
# Bars are now side-by-side, aka "dodged"
ggplot(sparrows, aes(x = Age, fill = Survival)) +
  geom_bar(position = position_dodge())
```

4. Again modify this barplot by making it *horizontal* by mapping `Age` to the Y axis *instead of the X.* `ggplot2` is exceptionally flexible!
```{r bar_4, exercise = TRUE}

```

```{r bar_4-solution}
ggplot(sparrows, aes(y = Age, fill = Survival)) +
  geom_bar(position = position_dodge())
```

5. Let's see what happens when you *accidentally use the wrong geom.* Again, make this plot (vertical version, not horizontal), but use `geom_col()` instead, by running the code below. You'll get an error! Read and understand why the error is there (*and check out the hint too!*)

```{r bar_5, exercise = TRUE, eval=FALSE}
ggplot(sparrows, aes(x = Age, fill = Survival)) +
  geom_col(position = position_dodge())
```

<div id="bar_5-hint">
`geom_col` NEEDS both x and y, so `ggplot2` will refuse to plot anything until the required aesthetics are given. **THIS DOES NOT ALWAYS MEAN YOU NEED TO JUST ADD A Y AND YOUR PROBLEMS ARE SOLVED.** It often means you have used the wrong geom for the type of plot you want to make!!
</div>


#### Switching gears

Now, let's practice `geom_col()` using these other datasets I have made that are more suited for the geom. These datasets contain the actual counts for how many of teach type of sparrow are in the dataset (soon, you will learn to manipulate data to create this sort of thing yourself!!). **Explore the contents of these three data frames before proceeding!**

```{r sparrows-summary-demo, exercise=TRUE}
sparrows_survival_precounted
sparrows_age_precounted
sparrows_precounted
```

6. Using the `sparrows_survival_precounted`, plot the number of dead and alive birds with `geom_col()`.

> **Hint:** The column `n` contains the count that you LITERALLY want along the Y-axis.
...
```{r bar_6-solution}
ggplot(sparrows_survival_precounted, aes(x = Survival, y = n)) +
  geom_col()
```

7. Using the `sparrows_age_precounted`, plot the number of dead and alive birds with `geom_col()`. This time, make the bars a single *color and fill* of your choosing! 

> **Hint:** No mapping here, "just" a color and fill!
...
```{r bar_7-solution}
ggplot(sparrows_age_precounted, aes(x = Age, y = n)) +
  geom_col(fill = "darkseagreen3", color = "dodgerblue4")
```


8. Let's again see what happens if the wrong geom is accidentally used. See the **Hint** for an explanation!

```{r bar_8, exercise=TRUE}
ggplot(sparrows_age_precounted, aes(x = Age, y = n)) +
  geom_bar(fill = "darkseagreen3", color = "dodgerblue4")
```

<div id="bar_8-hint">
`geom_bar()` does calculations to count data using a special function called `stat_count()`. In fact, many geoms use stat functions implicitly - `geom_histogram()` uses `stat_bin()`, and `geom_density()` uses `stat_density()`. These stat functions perform the data transformations necessary to build the plot. Here, your error says `stat_count()` doesn't know how to count 2 axes! One only, please! 
</div>


9. Finally, use `geom_col()` to create a *dodged* barplot of the bird counts for `Survival` and `Sex` shown in `sparrows_precounted`.

> Hint: You want to FILL by one variable, and set the other variable to the X axis.

```{r bar_9, exercise=TRUE}

```

```{r bar_9-solution}
ggplot(sparrows_precounted, aes(x = Age, y = n, fill = Sex)) +
  geom_col(position = position_dodge())
```
