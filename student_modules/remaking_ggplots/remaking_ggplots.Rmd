...
ggplot(backpacks, aes(x = BackpackWeight, fill = Sex)) +
    ...
    labs(x="Backpack Weight (lb)", y = "Count")
```
**Hint:** Instead of our new graph being filled by "firebrick," it is now filled by a distinction in `Sex`
...
ggplot(CO2, aes(x = Type, y = conc)) +
  geom_boxplot()
```
**Hint:** This is the most basic boxplot, a discrete X and continous Y.
...
```
**Hint:** The differing locations are under the variable `Type`.
